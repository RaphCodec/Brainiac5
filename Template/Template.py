...
logger.add('Template.log.txt', format="{time:YYYY-MM-DD HH:mm:ss} {level} | {message}", level="INFO", mode='w')
...
def Transform(df):
    ...
    Use the num_cols and date_cols lines if you want to transform multiple columns at once.
    '''
    num_cols = [
    ...
    # qry = b5.CreateTable(df,DEST_TABLE,primary=['PK_Column'],primaryName=f'PK__{DEST_TABLE}__ID',saveQuery=True)
    # ic(qry)

    #this line helps ensure there are no errors inserting NULLs into SQL and should always come after
...
def Load(df):
    ...
    DIT_QRY = f'SELECT PK_COLUMN FROM {DEST_TABLE}'
    DIT = pd.read_sql(DIT_QRY, conn)
    
    df_insert = df[~df['PK_COLUMN'].isin(DIT['PK_COLUMN'])]
    df_update = df[df['PK_COLUMN'].isin(DIT['PK_COLUMN'])]
    
    insert = b5.MakeInsertQuery(df_insert.columns.tolist(),DEST_TABLE)
    update = b5.MakeUpdateQuery(df_update.columns.tolist(),DEST_TABLE, where=['PK_COLUMN'])
