...
def IntType(column):
    ...
    if min_val in range(0, 256) and max_val in range(0, 256):
        ...
        min_val in range(-2_147_483_648, 2_147_483_648)
        or max_val in range(-2_147_483_648, 2_147_483_648)
    ) and (
...
class Query:
    ...
    def CreateTable(
        ...
        if foreign and not foreignTable:
            raise ValueError(
                "foreignTable must not be None if a foreign key is to be added"
            )
        ...
        for column, dtype in zip(columns, dtypes):
            sql_type = sql_data_types.get(
                str(dtype), "VARCHAR(max)"
            )  # defaulting to VARCHAR(max) if no datatype is matched
            if dtype == "object":
                ...
                create_table_query += (
                    f"    [{column}] DECIMAL({max_before + max_after},{max_after}),\n"
                )
            ...
            if (
                primary != None and column in primary
            ) or column == primary:  # ensures that primary key columns are not null
        ...
        if primary is not None:
            primary_key_constraint = (
                f"CONSTRAINT [{primaryName}] PRIMARY KEY "
                if primaryName
                else "PRIMARY KEY "
            )
        ...
        if (
            foreign is not None
            and foreignTable is not None
            and foreignRelated is not None
        ):
            foreign_key_constraint = (
                f"CONSTRAINT [{foreignName}] FOREIGN KEY "
                if foreignName
                else "FOREIGN KEY "
            )
        ...
        if unique is not None:
            unique_constraint = (
                f"CONSTRAINT [{uniqueName}] UNIQUE " if uniqueName else "UNIQUE "
            )
    ...
    def Update(self, where: list) -> str:
        ...
        columns = "\n\t,".join(
            [
                f"[{value}] = ?"
                for value in self.df.columns.tolist()
                if value not in where
            ]
        )

        where_clause = "\n\tAND\n\t".join(
            [f"[{value}] = ?" for value in where]
        )  # Making where clause
